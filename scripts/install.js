...
if (
  process.env['NODE_LIBPNG_SKIP_BINARY_DOWNLOAD_FOR_CI'] ||
  process.env['npm_config_node_libpng_skip_binary_download_for_ci']
) {
  return;
}
...
request
  ...
  .on('response', (response) => {
    ...
    if (response.statusCode === 404) {
      ...
      throw new Error(
        `Error downloading binaries for node-libpng ${packageVersion}. Received status code ${response.statusCode}`
      );
    }
    destination.close();
